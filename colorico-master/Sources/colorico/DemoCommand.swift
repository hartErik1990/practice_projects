...
final class DemoCommand: Command {
    ...
    func run(using context: CommandContext, signature: Signature) throws {
        ...
        let command = AnyCommand.run(using: &<#T##CommandContext#>)
        context.input.executable
        command.run(using: CommandContext)
        context.console.run(AnyCommand, input: <#T##CommandInput#>)
        ...
        let this = context.console.ask("ascasc".consoleText(.success))
        if signature.color {
        ...
        let input = context.console.input(isSecure: false)
        print(input)
